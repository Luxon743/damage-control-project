...
const fechaFin = ref('')
const empresaSolicitanteId = ref('') // Debe ser una empresa contratista
const empresaContratanteId = ref('') // Debe ser una empresa contratante
  ...
  <form @submit.prevent="enviar" class="space-y-6">
    <div class="bg-slate-50 p-8 rounded-3xl border border-slate-200 shadow-inner space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <InputTexto v-model="titulo" etiqueta="Título del trabajo" obligatorio placeholder="Ej: Mantenimiento de torre" />
        <InputTexto v-model="ubicacion" etiqueta="Ubicación / Sector" />
        <InputTexto v-model="fechaInicio" tipo="date" etiqueta="Fecha inicio" obligatorio />
        <InputTexto v-model="fechaFin" tipo="date" etiqueta="Fecha fin" obligatorio />
      </div>

      <TextAreaInput v-model="descripcion" etiqueta="Descripción de la tarea" />

      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <SelectInput
          v-model="empresaSolicitanteId"
          :opciones="empresasContratistas"
          etiqueta="Empresa solicitante (PyME)"
          obligatorio
        />
        <SelectInput
          v-model="empresaContratanteId"

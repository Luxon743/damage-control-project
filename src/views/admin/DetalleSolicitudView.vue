...
const { pedidos, evaluarPedido } = usePermisos()
...
const versionesSeguras = computed(() => pedido.value?.versiones ?? [])
...
const confirmarAprobacion = () => {
    if (confirm('¿Estas seguro de aprobar este permiso de trabajo?')) {
        evaluarPedido(idPedido, 'aprobado')
        router.push('/admin')
    }
}
...
const confirmarRechazo = () => {
    ...
    evaluarPedido(idPedido, 'rechazado', comentarioRechazo.value)
    router.push('/admin')
}
</script>
...
<template>
    <div v-if="pedido && versionActual" class="max-w-5xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b-2 border-slate-200 pb-4 gap-4">
            <div>
                <div class="flex items-center gap-3">
                    <h1 class="text-xl font-black text-slate-800 uppercase tracking-wide">Auditoria {{ pedido.id }}</h1>
            ...
            <button @click="router.push('/admin')" class="px-5 py-2 rounded-full border-2 border-slate-300 text-xs font-bold uppercase hover:bg-slate-50 transition">
                Volver
            </button>
        </div>
        ...
        <div class="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
            ...
            <div v-if="pedido.estado === 'pendiente' && indiceVersionSeleccionada === totalVersiones - 1" class="mt-8 pt-6 border-t-2 border-slate-200">
                
                <div v-if="!modoRechazo" class="flex items-center justify-end gap-4">
                    <button 
                        @click="modoRechazo = true"
                ...
                <div v-else class="bg-rose-50 border-2 border-rose-200 p-6 rounded-2xl space-y-4">
                    ...
                    <textarea 
                        ...
                        class="w-full bg-white border-2 border-rose-200 rounded-xl px-4 py-3 focus:border-rose-400 outline-none transition text-sm shadow-sm"
                    ></textarea>

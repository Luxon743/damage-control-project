...
const { permisos, revisarPermiso } = usePermisos()
...
// Banco de preguntas (para mostrar info)
...
<template>
  <div v-if="permiso && versionActual" class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
          ...
          >
            ...
            <div class="flex items-center gap-2.5 bg-emerald-50 border border-emerald-200 p-3 rounded-xl">
              ...
              <span class="text-sm font-semibold text-emerald-800">
                {{ bancoPreguntas[permiso.tipoTrabajo.nombre.toLowerCase()]?.[preId]?.res[opcionId] ?? opcionId }}
              </span>
            </div>
          </div>
        </div>
      ...
      <div v-if="permiso.estado === 'pendiente' && indiceVersionSeleccionada === totalVersiones - 1" class="mt-8 pt-6 border-t-2 border-slate-200">

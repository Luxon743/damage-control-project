...
<template>
    <div class="max-w-5xl mx-auto space-y-8" v-if="permiso">
        <div class="border-b-2 border-slate-200 pb-4">
            <div class="flex items-center gap-3">
                ...
                <span class="text-[10px] font-bold text-white bg-amber-500 px-2 py-0.5 rounded-md uppercase animate-pulse">
    ...
    <div v-else class="text-center py-20 bg-slate-50 border border-slate-200 rounded-3xl max-w-5xl mx-auto">
        <p class="text-slate-400 font-medium">No se encontró el permiso que se desea editar o no está disponible.</p>
        <button @click="router.push({ name: 'mis-permisos' })" class="mt-4 text-xs font-bold uppercase text-indigo-600 hover:underline cursor-pointer">
            Volver a la lista
        </button>
    </div>
</template>